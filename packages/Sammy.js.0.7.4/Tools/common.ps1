...
# Extract the version number from the sammy file in the package's content\scripts folder
$packageScriptsFolder = Join-Path $installPath Content\Scripts
$sammyFileName = Join-Path $packageScriptsFolder "sammy-*.js" | Get-ChildItem -Exclude "*.min.js","*-vsdoc.js" | Split-Path -Leaf
$sammyFileNameRegEx = "sammy-((?:\d+\.)?(?:\d+\.)?(?:\d+\.)?(?:\d+)).js"
$sammyFileName -match $sammyFileNameRegEx
$ver = $matches[1]

$intelliSenseFileName = "sammy-$ver.intellisense.js"
